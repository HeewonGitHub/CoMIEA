...
This repository provides the R implementation and toy dataset for **CoMIEA**.

CoMIEA identifies biological pathways whose molecular interplays change coherently along a continuous phenotype by integrating topology-weighted regulatory activity, phenotype-coherence scoring, and permutation-based pathway enrichment.
...
└── ToyDATA_CoMIEA/
    ...
    ├── M.csv
    ├── PWgenes.csv
    └── BETA_Sample1.csv ... BETA_Sample50.csv
```
...
```text
Gene expression and continuous phenotype
                ↓
...
- Directed outgoing eigenvector centrality
- Participation coefficient with Louvain community detection
...
The toy expression matrix, phenotype (modulator), pathway genes, and sample-specific network coefficients are loaded.
...
| `EC` | Outgoing eigenvector centrality |
| `PC_normalized` | Normalized participation coefficient |
| `CM_normalized` | Normalized communicability |
| `THETA` | Topology-weighted regulatory activity |
| `S` | Phenotype-coherence score |
...
For reproducible permutation results, set
