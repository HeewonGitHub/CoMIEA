...
The final `Pvalue` evaluates whether the genes in the tested pathway have a larger mean phenotype-coherence score than randomly selected gene sets of the same size.

A small p-value indicates that the molecular interplays associated with the tested pathway change coherently across the continuous phenotypic spectrum beyond what would be expected by chance.

---

## Reproducibility

The permutation test uses random gene sampling. Add the following line before the permutation procedure to reproduce the same result:
...
The sample-specific `BETA` matrices are treated as predefined inputs in this example. Reproducibility of the complete analysis therefore also depends on using the same expression matrix, phenotype values, pathway genes, and sample-specific coefficient matrices.

---

## Notes

- The example assumes `p = 250` genes and `n = 50` samples.
- `NoPM = 201` corresponds to one observed pathway score and 200 gene-set permutation scores.
- The diagonal elements of each adjacency matrix are removed before topology calculations.
- Absolute regulatory coefficients are used for the network-topology measures.
- The signed coefficient is retained when calculating the regulatory effect.
- The tested pathway is represented by the genes listed in `PWgenes.csv`.
